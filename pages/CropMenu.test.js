...
jest.mock("../lib/supabase", () => ({
  supabase: {
    from: jest.fn(() => ({
      ...
      update: () => ({
        eq: () => ({ error: null }), // pretend update always succeeds
      }),
      insert: () => ({ error: null }), // pretend insert succeeds
      delete: () => ({ eq: () => ({ error: null }) }), // pretend delete succeeds
    })),
    channel: () => ({
...
describe("CropMenu", () => {
  ...
  test("clicks Delete on an existing crop without errors", async () => {
    ...
    // (Supabase delete is mocked to succeed; component does not crash)
